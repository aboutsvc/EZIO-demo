...
export function MainBanner() {
  ...
  return (
    <section
      aria-label={t(ui.mainBanner.label)}
      aria-roledescription="carousel"
      className="relative h-[400px] overflow-hidden bg-brand-navy sm:h-[440px] lg:h-[500px]"
    >
      {/* 배경 아트워크 (슬라이드별 교체) */}
      ...
      <div className="relative mx-auto flex h-full max-w-[1200px] items-center px-4">
        <div key={`${slide.id}-copy`} className="slide-fade max-w-[36rem]">
          <p className="inline-block border border-white/35 px-3 py-1 text-[0.75rem] font-semibold tracking-[0.1em] text-white/85">
            {t(slide.eyebrow)}
          </p>
          <h2 className="mt-4 whitespace-pre-line text-[1.75rem] font-bold leading-[1.25] tracking-[-0.03em] text-white sm:text-[2.25rem] lg:text-[2.625rem]">
            {t(slide.headline)}
          </h2>
          <p className="mt-3 text-[0.9375rem] leading-relaxed text-white/80 sm:text-base">
            {t(slide.sub)}
          </p>
          <Link to={slide.cta.to} className="btn-blue mt-6 h-11 px-6 text-[0.9375rem]">
            {t(slide.cta.label)}
      ...
      <div className="absolute inset-x-0 bottom-5 z-10">
        <div className="mx-auto flex max-w-[1200px] items-center gap-3 px-4">
          <button
            type="button"
            onClick={() => go(index - 1)}
            aria-label={t(ui.mainBanner.prev)}
          ...
          <div className="flex items-center gap-2">
            {bannerSlides.map((s, i) => (
              <button
                ...
                type="button"
                onClick={() => go(i)}
                aria-label={`${i + 1}${t(ui.mainBanner.goTo)}`}
          ...
          <button
            type="button"
            onClick={() => go(index + 1)}
            aria-label={t(ui.mainBanner.next)}
            className="flex h-8 w-8 items-center justify-center border border-white/40 text-white/85 hover:bg-white/15"
          >
            <span aria-hidden="true">›</span>
          </button>
          <button
            ...
            aria-label={playing ? t(ui.mainBanner.pause) : t(ui.mainBanner.play)}
