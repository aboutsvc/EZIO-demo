 ...
 * 하위 메뉴가 없는 단일 페이지(주요 고객·수행실적 / 채용정보)는 LNB 없이 전체 폭을 사용한다.
 */
...
export function SubPage({ path, children }: SubPageProps) {
  const navigate = useNavigate();
  const meta = pageMeta[path];
  const section = navSections[meta.sectionIndex];
  const hasLnb = section.children.length > 0;
          ...
          {hasLnb ? (
            <>
              ...
              <span className="font-semibold text-ink">
                {activeChild ? activeChild.label : meta.title}
              </span>
            </>
          ) : (
      ...
      <div className="mx-auto flex max-w-[1200px] flex-col gap-8 px-4 py-8 lg:flex-row lg:gap-10 lg:py-12">
        {hasLnb ? (
          <div className="lg:w-[210px] lg:shrink-0">
            {/* Desktop LNB */}
            <nav aria-label={section.label} className="hidden lg:block">
